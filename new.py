...
for_each_cell = 25
block_opp_without_center = 50
block_opp_with_center = 20
...
def evaluate_block(block, myflag):
	...
	elif (block[0][0] == oppflag and block[1][1] == oppflag and block[2][2] == myflag) or (block[0][0] == myflag and block[1][1] == oppflag and block[2][2] == oppflag):
		utility += block_opp_without_center
	elif block[0][0] == oppflag and block[1][1] == myflag and block[2][2] == oppflag :
		utility += block_opp_with_center

	elif (block[0][2] == oppflag and block[1][1] == oppflag and block[2][0] == myflag) or (block[0][2] == myflag and block[1][1] == oppflag and block[2][0] == oppflag) :
		utility += block_opp_without_center
	elif block[0][2] == oppflag and block[1][1] == myflag and block[2][0] == oppflag :
		utility += block_opp_with_center

	elif (block[0][1] == oppflag and block[1][1] == oppflag and block[2][1] == myflag) or (block[0][1] == myflag and block[1][1] == oppflag and block[2][1] == oppflag) :
		utility += block_opp_without_center
	elif block[0][1] == oppflag and block[1][1] == myflag and block[2][1] == oppflag :
		utility += block_opp_with_center

	elif (block[0][0] == oppflag and block[0][1] == oppflag and block[0][2] == myflag) or (block[0][0] == myflag and block[0][1] == oppflag and block[0][2] == oppflag) or (block[0][0] == oppflag and block[0][1] == myflag and block[0][2] == oppflag):
		utility += block_opp_without_center

	elif (block[2][0] == oppflag and block[2][1] == oppflag and block[2][2] == myflag) or (block[2][0] == myflag and block[2][1] == oppflag and block[2][2] == oppflag) or (block[2][0] == oppflag and block[2][1] == myflag and block[2][2] == oppflag):
		utility += block_opp_without_center

	elif (block[0][0] == oppflag and block[1][0] == oppflag and block[2][0] == myflag) or (block[0][0] == myflag and block[1][0] == oppflag and block[2][0] == oppflag) or (block[0][0] == oppflag and block[1][0] == myflag and block[2][0] == oppflag):
		utility += block_opp_without_center
...
def evaluate_grid(bs):
	...
	for i in xrange(0,9,3):
		...
		if v < const:
			...
			ret += v**3
		#print "v-const ",v-const
		#ret += (3*DRAW)*exp((v-(const))/(2*const))*v
	for i in xrange(3):
		v=1
		for i in xrange(0,9,3):
				v *= bs[i+j]
...
class Player1:
		...
		def move(self,temp_board,temp_block,old_move,myflag):
			...
			if old_move == (-1, -1):
				...
				if self.number_of_moves_taken <= 5:
					for cell in cells:
						search_moves_allowed = determine_blocks_allowed(cell, temp_block)
